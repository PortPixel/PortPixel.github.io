var aiControlledGameExportVars;
function aiControlledGameLoader() {
  var disableKeyboardControls = true;
  "use strict";
  aiControlledGameExportVars = {};
  var cl = console.log;
  "use strict";
  ...
  function addLevels() {
    var i;
    levelDisplayNames = {};
    var nameBase = "aiControlledMap";
    var nameTemp;
    for (i = 0; i < 50; i++) {
      nameTemp = nameBase + String(i + 1);
      aiControlledGameExportVars.addLevelToPlayingOrder(nameTemp);
      levelDisplayNames[nameTemp] = nameTemp;
    }
  }
  ...
  function getMapForLevelID(levelID, addCells, includeInitialCellPositions) {
    ...
    var tempMap = aiControlledGameExportVars.createNewMap(null, null);
    ...
    var addCellToMapWrapper = function(x, y, size, xv, yv) {
      ...
      if (addCells) {
        aiControlledGameExportVars.createNewCell(x, y, size, xv, yv);
      }
    };
    if (levelID.startsWith("aiControlledMap")) {
      var numCellsToCreate = Number(levelID.substr("aiControlledMap".length));
      ...
      for (i = 0; i < numCellsToCreate; i++) {
        ...
        addCellToMapWrapper((seededRandomForMap.getRand() * 2 - 1) * 80, (seededRandomForMap.getRand() * 2 - 1) * 80, aiControlledGameExportVars.options.cellSize, Math.sin(dirTemp) * speedTemp, Math.cos(dirTemp) * speedTemp);
      }
    } else {
      console.error("ERROR: for this function to be called, level id should start with aiControlledMap - may be caused by collison/overwriting between normal levels data and the ai controlled mouse data?");
    }
  ...
  var inputType = 0;
  var colourScheme = {rotatingArm:"hsla(0, 0%, 60%, 1)", circularBooster:{one:"hsla(0, 0%, 38%, 1)", two:"hsla(0, 0%, 56%, 1)"}, map:{mainColor:"hsla(0, 0%, 73%, 1)"}, background:"hsla(240, 100%, 5%, 1)"};
  var cells = [];
  ...
  var gameArea = {keys:[], canvas:document.createElement("canvas"), start:function() {
  ...
  function renderAnimationForMorphingMapsFromAnimationDataObj(animationData, frameCompletionFraction, extraAnimatedOffset) {
    ...
    aiControlledGameExportVars.currentlyMorphingMapPartiallyMorphed = tempMap;
  ...
  function renderGameSuccessBox() {
    ...
    if (ui.scrollingToNextDifferentLevel.animation.curScrolling) {
      ...
      if (ui.scrollingToNextDifferentLevel.animation.frame >= ui.scrollingToNextDifferentLevel.animation.frameDuration) {
        renderText = false;
        aiControlledGameExportVars.currentlyMorphingMapAndCells = false;
        if (ui.scrollingToNextDifferentLevel.animation.frame === ui.scrollingToNextDifferentLevel.animation.frameDuration) {
        ...
        aiControlledGameExportVars.currentlyMorphingMapAndCells = true;
  ...
  function render() {
    ...
    if (ui.scene === 0 || (ui.scene === 1 || ui.scene === 2)) {
      ...
      if (ui.scene === 0) {
        ...
        if (aiControlledGameExportVars.hasBeenUnloaded) {
          return;
        }
    ...
    ctx.fillStyle = "red";
    ctx.fillRect(aiMousePos.x, aiMousePos.y, 10, 10);
  }
          ...
          if (anyPointTooFarFromLine) {
            ...
            if (Math.min(Math.abs(entryLineData.segmentEndPoints[0].index - exitLineData.segmentEndPoints[0].index), map.points.length - Math.abs(entryLineData.segmentEndPoints[0].index - exitLineData.segmentEndPoints[0].index)) < 1) {
              ...
              if (discardedMapCellCount !== 0) {
                ...
                changeSceneToCellDeadAndMovingCameraUpToOveriewOfMap();
              } else {
                map = bestTempMap;
  ...
  function updateCamera() {
    if (options.reduceSizeOfCanvas) {
      centerX = window.innerWidth / 2 * 0.5;
      centerY = window.innerHeight / 2 * 0.5;
    } else {
  ...
  var aiMousePos = {x:100, y:0, xv:0, yv:10};
  function updateMouseHistoryAIControlled(offset) {
    var i;
    var mapMeanPos = {x:0, y:0};
    var point;
    for (i = 0; i < map.points.length; i++) {
      point = map.points[i];
      mapMeanPos.x += point.x;
      mapMeanPos.y += point.y;
    }
    mapMeanPos.x /= map.points.length;
    mapMeanPos.y /= map.points.length;
    mapMeanPos.x = offset.x;
    mapMeanPos.y = offset.y;
    ctx.fillStyle = "red";
    ctx.fillRect(mapMeanPos.x, mapMeanPos.y, 10, 10);
    var mapMeanPosProjected = projectPoint(mapMeanPos);
    var dirTemp = getDirection(aiMousePos.x - mapMeanPosProjected.x, aiMousePos.y - mapMeanPosProjected.y);
    var distTemp = getDistance(aiMousePos.x - mapMeanPosProjected.x, aiMousePos.y - mapMeanPosProjected.y);
    aiMousePos.xv -= Math.sin(dirTemp) * 3 * Math.tanh(distTemp / 100) * 100 * 0.01 * 1;
    aiMousePos.yv -= Math.cos(dirTemp) * 3 * Math.tanh(distTemp / 100) * 100 * 0.01 * 1;
    aiMousePos.xv *= 0.98;
    aiMousePos.yv *= 0.98;
    aiMousePos.x += aiMousePos.xv;
    aiMousePos.y += aiMousePos.yv;
    aiMousePos.x += (Math.random() - 0.5) * 100 * 1;
    aiMousePos.y += (Math.random() - 0.5) * 100 * 1;
    if (gameArea.keys[81]) {
      aiMousePos.x = mouseposx;
      aiMousePos.y = mouseposy;
    }
    var unProjectedMousePos = {x:cameraX - aiMousePos.x / zoom - offset.x, y:cameraY - aiMousePos.y / zoom - offset.y};
    var mouseInsideMap = checkIfUnprojectedPointIsInsideMap(unProjectedMousePos);
    if (mousePosHistory.length < 3) {
      mousePosHistory.unshift({x:unProjectedMousePos.x, y:unProjectedMousePos.y, velSquared:0, insideMap:mouseInsideMap, mouseState:mouseState});
    } else {
  ...
  function updateGameArea() {
    ...
    if (false) {
      ...
      var offset = getMapStackOffsetCurLevel();
      updateMouseHistoryAIControlled(offset);
      var i;
  ...
  function unload() {
    aiControlledGameExportVars.hasBeenUnloaded = true;
    clearInterval(gameArea.interval);
    DOMBody.removeChild(canvas);
    cl("a");
  }
  aiControlledGameExportVars.hasBeenUnloaded = false;
  aiControlledGameExportVars.unload = unload;
  aiControlledGameExportVars.gameArea = gameArea;
  aiControlledGameExportVars.addLevelToPlayingOrder = addLevelToPlayingOrder;
  aiControlledGameExportVars.createNewMap = createNewMap;
  aiControlledGameExportVars.options = options;
  aiControlledGameExportVars.createNewCell = createNewCell;
  aiControlledGameExportVars.startGame = startGame;
  aiControlledGameExportVars.canvas = gameArea.canvas;
  aiControlledGameExportVars.currentlyMorphingMapAndCells = false;
  delete aiControlledGameExportVars.map;
  delete aiControlledGameExportVars.cells;
  Object.defineProperty(aiControlledGameExportVars, "map", {get:function() {
    if (aiControlledGameExportVars.currentlyMorphingMapAndCells) {
      return aiControlledGameExportVars.currentlyMorphingMapPartiallyMorphed;
    } else {
      return map;
    }
  }});
  Object.defineProperty(aiControlledGameExportVars, "cells", {get:function() {
    return cells;
  }});
  Object.defineProperty(aiControlledGameExportVars, "ui", {get:function() {
    return ui;
  }});
  addLevels();
